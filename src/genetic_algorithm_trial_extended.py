...
# Load extended city distance matrix (only on root process)
if rank == 0:
    file_path = os.path.join(os.path.dirname(__file__), '../data/city_distances_extended.csv')
    distance_matrix = pd.read_csv(file_path).to_numpy()
    num_nodes = distance_matrix.shape[0]
    population_size = 10000
    num_generations = 200
    mutation_rate = 0.1
    stagnation_limit = 20  # Increased stagnation limit for a larger dataset
...
for generation in range(num_generations):
    ...
    if rank == 0:
        ...
        if stagnation_counter >= stagnation_limit:
            ...
            mutation_rate = min(0.5, mutation_rate * 1.25)  # Increase mutation rate
            population = generate_unique_population(population_size, num_nodes)
            stagnation_counter = 0
        else:
            # Selection and crossover
            selected = select_in_tournament(population, all_fitness_values, tournament_size=6)
            offspring = []
